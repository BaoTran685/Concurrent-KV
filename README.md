...
- Newline-delimited (`\n`) TCP command protocol for communicating between clients and server.
- Fixed-size worker thread pool using the producer-consumer pattern to service multiple clients concurrently.
- Shared client queue using `std::mutex` and `std::conditional_variable`.
- Multiple synchronization strategies for the in-memory KV engine:
    ...
    - Sharded `std::shared_mutex` *(sharded locking)*: paritition keys accross independently locked shards to reduce lock contention.
...
The server is divided into four main components: the TCP networking, worker thread pool, concurrent in-memory key-value engine, and write-ahead logging.
...
Workers wait on a `std::conditional_variable` while the client queue is empty. When a new connection is added, `notify_one()` wakes up one worker and the worker removes that `client_fd` from the queue and becomes responsible for servicing that client.

Each worker remains assigned to its client until the client disconnects. With three workers, the server can service up to three client connections simultaneously. Additional connections remain in the queue until a worker becomes available.

### Concurrent In-Memory Engine
Commands received from clients are parsed and then sent to the in-memory key-value store.

The project implements three thread sychronization strategies:
...
For a `SET` or `DELETE` operation, the server first serializes the command and appends it to the WAL file before applying the change to the in-memory key-value store.

We prioritize writing to WAL over making update to the in-memory store because in case of a sudden shutdown, if the operation is written in WAL, we can recover it later.

`GET` operations do not modify database state and therefore do not need to be written to the WAL.

### Crash Recovery
The in-memory key-value store, as its name, only stays in RAM and when the process is terminated, all contents will disappear. The WAL acts as a durable source of mutation history.

When the server starts, it reads the WAL sequentially and replays each valid `SET` and `DELETE` operation. Because mutations/operations are replayed in the same order in which they were originally logged, the reconstructed in-memory database should reach the same state that it was right before shutdown (they should have the same key-value pairs).
